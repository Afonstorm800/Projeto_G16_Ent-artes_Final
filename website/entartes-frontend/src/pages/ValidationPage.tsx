...
const initialBookings: BookingRequest[] = [
  { id: 1, student: "Rita Gomes", guardian: "Carla Gomes", modality: "Ballet Clássico", format: "Individual", day: "Ter", time: "16:00", professor: "Pedro Santos", objective: "Preparar audição CNB", receivedAt: "12 Mar · 09:14", profStatus: "pending", directionStatus: "n/a", studio: null },
  { id: 2, student: "João Silva", guardian: "Ana Silva", modality: "Dança Contemporânea", format: "Duo", day: "Qua", time: "15:00", professor: "Maria Costa", objective: "Trabalhar peça de exame", receivedAt: "12 Mar · 11:02", profStatus: "accepted", directionStatus: "pending", studio: null },
  { id: 3, student: "Sara Mendes", guardian: "Luís Mendes", modality: "Jazz", format: "Individual", day: "Qui", time: "14:00", professor: "Pedro Santos", objective: "Coreografia para festival", receivedAt: "13 Mar · 08:31", profStatus: "pending", directionStatus: "n/a", studio: null },
  { id: 4, student: "Miguel Ferreira", guardian: "Carla Gomes", modality: "Hip Hop", format: "Trio", day: "Sex", time: "16:00", professor: "Ana Lopes", objective: "Preparar showcase de fim de ano", receivedAt: "11 Mar · 17:45", profStatus: "accepted", directionStatus: "approved", studio: "Estúdio 1" },
];
    ...
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-3xl font-bold text-foreground">
          {role === "professor" ? "Pedidos & Confirmações" : role === "encarregado" ? "Pedidos do Encarregado" : "Validações"}
        </h1>
        <p className="text-muted-foreground mt-1">
          ...
          {role === "encarregado" && "Acompanhamento dos pedidos e confirmação das sessões realizadas"}
        </p>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted">
                  <th className="text-left p-3 font-semibold text-muted-foreground">Educando</th>
                  ...
                  <th className="text-left p-3 font-semibold text-muted-foreground">Recebido</th>
              ...
              <tbody className="divide-y divide-border">
                {profRequests.length === 0 && (
                  <tr><td colSpan={7} className="p-6 text-center text-muted-foreground">Sem pedidos pendentes.</td></tr>
                )}
                {profRequests
                  .slice()
                  .sort((a, b) => a.id - b.id)
                  ...
                  <motion.tr key={b.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}>
                    ...
                    <td className="p-3 text-muted-foreground text-xs">{b.receivedAt}</td>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {directionRequests.length === 0 && (
                  <tr><td colSpan={8} className="p-6 text-center text-muted-foreground">Sem pedidos a aguardar aprovação.</td></tr>
                )}
                {directionRequests
                  .slice()
                  .sort((a, b) => a.id - b.id)
                  ...
                  <motion.tr key={b.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}>
                    ...
                    <td className="p-3 text-right">
                      <div className="flex gap-2 justify-end">
                        ...
                        <Button size="sm" variant="outline" onClick={() => directionAction(b.id, "rejected")}>Desaprovar</Button>
                      </div>
                    </td>
                  </motion.tr>
                ))}
