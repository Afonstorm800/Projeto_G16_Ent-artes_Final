...
import { Plus, X, Package, ArrowLeftRight, ShoppingBag, Calendar as CalendarIcon, AlertTriangle, Upload } from "lucide-react";

type InventoryTab = "items" | "loans" | "sales";
...
const InventoryPage = () => {
  ...
  const tabs: { id: InventoryTab; label: string }[] = isDirection
    ...
    : [
        { id: "items", label: "Inventário" },
        { id: "loans", label: "Alugueres do encarregado" },
        { id: "sales", label: "Compras do encarregado" },
      ];

  const [tab, setTab] = useState<InventoryTab>("items");
  const [filter, setFilter] = useState<"all" | ItemType>("all");
  const [, setContributions] = useState(initialContributions);
  const [loans, setLoans] = useState(initialLoans);
  const [sales, setSales] = useState(initialSales);
  const [showContributeModal, setShowContributeModal] = useState(false);
  const [actionItem, setActionItem] = useState<{ item: Item; action: "rent" | "buy" } | null>(null);
  // Add-item modal state
  const [lists, setLists] = useState<string[]>(["Os meus figurinos", "Acessórios da Rita"]);
  ...
  // contributions removed from UI; setter kept to avoid unused init
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-accent/30 bg-accent/5 p-4 flex items-start gap-3">
        ...
        <p className="text-xs text-foreground leading-relaxed">
          O Ent'Artes <strong>não se responsabiliza</strong> por falhas, perdas ou danos em alugueres ou vendas.
          Este é um serviço opcional disponibilizado à comunidade da escola; a sua utilização é da responsabilidade de quem o utiliza.
        </p>
      </div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between">
                <h2 className="font-display text-xl font-semibold text-foreground">Adicionar Item</h2>
              ...
              <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1">
                <div>
                  <label className="text-sm font-medium text-foreground block mb-2">Lista</label>
                  {!creatingList ? (
                    <div className="flex gap-2">
                      <select
                        value={selectedList}
                      ...
                      <Button type="button" variant="outline" size="sm" onClick={() => setCreatingList(true)}>+ Criar nova</Button>
                    </div>
                  ) : (
                    <div className="flex gap-2">
                      <input
                ...
                <div>
                  <label className="text-sm font-medium text-foreground block mb-2">Fotografia</label>
                  <input
                    ref={fileInputRef}
                    type="file"
                    accept="image/*"
                  ...
                  {photoPreview ? (
                    <div className="relative">
                      <img src={photoPreview} alt="Pré-visualização" className="w-full h-40 object-cover rounded-md border border-border" />
                      <button onClick={() => setPhotoPreview(null)} className="absolute top-2 right-2 bg-card/90 rounded-full p-1 border border-border">
                        <X className="h-3 w-3" />
                      </button>
                    </div>
                  ) : (
                    <button
                      ...
                      className="w-full h-32 rounded-md border-2 border-dashed border-border bg-muted/30 hover:bg-muted flex flex-col items-center justify-center gap-1 text-muted-foreground"
                    >
              ...
              <Button className="w-full" onClick={() => { setShowContributeModal(false); setPhotoPreview(null); }}>Adicionar à Lista</Button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
